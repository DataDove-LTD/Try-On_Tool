 ...
 */
// Modified by DataDove LTD on 05/11/2025
...
if (!defined('WOO_FITROOM_PREVIEW_VERSION')) {
    define('WOO_FITROOM_PREVIEW_VERSION', '1.3.0');
}
...
add_action('rest_api_init', function () {
    register_rest_route('woo-tryontool/v1', '/wasabi-image', array(
        ...
        'callback' => function ($request) {
            ...
            // Fetch image through server proxy (server has Wasabi credentials)
            $server_api = 'https://tryontool.com/wp-json/tryontool/v1';
            
            error_log('Wasabi Image Proxy: Requesting from server for key: ' . $key);
            
            // Request image from server-side proxy endpoint
            $proxy_response = wp_remote_get($server_api . '/wasabi/image-proxy?key=' . urlencode($key), array(
                'timeout' => 30,
                'sslverify' => false,
            ));
            
            if (is_wp_error($proxy_response)) {
                error_log('Wasabi Image Proxy Error: ' . $proxy_response->get_error_message());
                
                // Fallback: Try direct fetch (if bucket is public)
                $wasabi_url = 'https://s3.eu-west-1.wasabisys.com/tryontool-uploads/' . $key;
                error_log('Wasabi Image Proxy: Fallback - Direct fetch from: ' . $wasabi_url);
                
                $response = wp_remote_get($wasabi_url, array(
                    'timeout' => 30,
                    'sslverify' => false,
                ));
                
                if (is_wp_error($response)) {
                    error_log('Wasabi Image Proxy Fallback Error: ' . $response->get_error_message());
                ...
                if ($response_code === 200) {
                    $imageData = $response_body;
                    $remoteContentType = wp_remote_retrieve_header($response, 'content-type');
                } else {
                    error_log('Wasabi Image Proxy Fallback Error: HTTP ' . $response_code);
                    return new WP_Error('not_found', 'Image not found', array('status' => 404));
                }
            } else {
                $response_code = wp_remote_retrieve_response_code($proxy_response);
                $response_body = wp_remote_retrieve_body($proxy_response);
                
                if ($response_code === 200) {
                    $imageData = $response_body;
                    $remoteContentType = wp_remote_retrieve_header($proxy_response, 'content-type');
                    error_log('Wasabi Image Proxy: Successfully fetched from server');
                } else {
                    error_log('Wasabi Image Proxy: Server returned HTTP ' . $response_code);
                    return new WP_Error('not_found', 'Image not found', array('status' => 404));
                }
            }
